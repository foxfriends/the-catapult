...
for(var i = 0; i < ds_grid_width(InventoryController.inv); i++) {
    if(inv_read(i, ITEMINDEX) != -1) {
        var str = "";
        if(inv_read(i, ITEMQTY) > 1) {
            str += "x" + string(inv_read(i, ITEMQTY)) + " ";
        }
        str += string_wrap(itemdb_read(inv_read(i, ITEMINDEX), ITEMNAME) + ": " + itemdb_read(inv_read(i, ITEMINDEX), ITEMDESC), background_get_width(bMenu) - 80);
